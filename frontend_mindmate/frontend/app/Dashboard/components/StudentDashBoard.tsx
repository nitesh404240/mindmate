"use client";
import { BookOpen, Search, Sparkles, FileText, Plus, ShoppingBag, ArrowRight } from "lucide-react";
...
<header className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
  <div>
    ...
    <p className="text-slate-500 text-sm font-medium mt-1">Ready to expand your second brain?</p>
  </div>
  ...
  <div className="flex items-center gap-3 flex-1 md:max-w-xl">
    ...
    >
      <Plus size={16} /> <span className="hidden sm:inline">Create Note</span>
    </button>
  </div>
</header>
  ...
  >
    ...
    <div className="flex items-center gap-4 z-10">
      ...
      <div className="text-left">
        <h3 className="font-black uppercase tracking-widest text-[10px] text-indigo-400">AI Research</h3>
        <p className="text-[11px] text-slate-500 font-medium">Generate deep-dive notes</p>
      </div>
    </div>
    ...
    <div className="z-10 bg-slate-800/50 p-2 rounded-lg group-hover:bg-indigo-600 group-hover:text-white transition-all">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pt-4">
        ...
        <div className="lg:col-span-5 grid grid-cols-2 gap-4">
          <StatCard label="Total Notes" value={notes.length} icon={FileText} colorClass="text-indigo-400" />
          <StatCard label="Purchased" value="7" icon={BookOpen} colorClass="text-purple-400" />
          ...
          <div className="col-span-2 bg-[#151921] border border-slate-800/60 rounded-[2rem] p-6 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-sm font-black text-white uppercase tracking-widest">Marketplace</h3>
              </div>
              <button onClick={() => router.push('/store')} className="text-[10px] font-black text-indigo-400 flex items-center gap-1 hover:gap-2 transition-all">
        ...
        <div className="lg:col-span-7">
          <div className="bg-[#151921] border border-slate-800/60 rounded-[2rem] p-6 h-full shadow-sm">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => router.push('/notes')} className="text-[10px] font-black text-slate-500 hover:text-white uppercase tracking-[0.2em] transition-colors">
                See All
              </button>
            </div>
